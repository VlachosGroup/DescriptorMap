...
def organize_excel_inputs(in_path='./inputs.xlsx'):
    ...
    sampling = out_dict['options']['sampling']
    out_dict['options']['n_jobs'] = get_tot_job_num(n_col=n_col, sampling=sampling)
    out_dict['options']['desc_shape'] = get_descriptors_shape(n_col=n_col,
                                                          sampling=sampling)
    out_dict['options']['n_concurrent'] = int(np.min([out_dict['options']['n_jobs'],
                                                      out_dict['options']['n_concurrent']]))
    return out_dict
